<template>
  <div>
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Roadmap categories</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Organise roadmap items into named categories</p>
      </div>
      <button @click="openNew" class="rounded-lg bg-gray-900 dark:bg-white px-4 py-2 text-sm font-semibold text-white dark:text-gray-900 hover:bg-gray-700 dark:hover:bg-gray-100 transition-colors">New category</button>
    </div>
    ...
    <div class="mt-8 rounded-xl bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-200 dark:ring-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
      <div v-if="!categories.length" class="px-6 py-10 text-center text-sm text-gray-400">No categories yet.</div>
      <div v-for="cat in categories" :key="cat.id" class="flex items-center justify-between px-5 py-4">
        <div class="flex items-center gap-3">
          <span class="w-3.5 h-3.5 rounded-full flex-shrink-0" :style="{ background: cat.color }" />
    ...
    <Teleport to="body">
      <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative z-10 w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6">
          <h2 class="text-base font-semibold text-gray-900 dark:text-white mb-5">{{ isNew ? 'New category' : 'Edit category' }}</h2>
          <div class="space-y-4">
            <div>
...
async function handleSave() {
  ...
  const res = isNew.value
    ? await useAxios.post('/api/admin/roadmap-categories', form.value)
...
async function load() {
  const res = await useAxios.get('/api/admin/roadmap-categories')
