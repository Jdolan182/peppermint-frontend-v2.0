  ...
  <div class="w-full min-h-screen bg-gray-50 dark:bg-gray-950">
    ...
    <div class="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800">
      <div class="mx-auto max-w-6xl px-6 lg:px-10 py-16 lg:py-24">
        <p class="text-sm font-semibold tracking-widest text-emerald-600 dark:text-emerald-400 uppercase mb-3">The Blog</p>
        <h1 class="text-5xl lg:text-6xl font-bold text-gray-900 dark:text-white leading-tight">
          {{ blogTitle }}
        </h1>
        <p v-if="blogDescription" class="mt-4 text-xl text-gray-500 dark:text-gray-300 max-w-xl">
          {{ blogDescription }}
        </p>
      </div>
    </div>
    ...
    <div class="mx-auto max-w-6xl px-6 lg:px-10 py-10">
      ...
      <div class="flex flex-wrap gap-2.5">
        <button
          ...
          :class="activeCategory === null
            ...
            : 'bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500 hover:text-gray-900 dark:hover:text-white'"
      ...
      <template v-if="loading">
        <div class="mt-16 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 animate-pulse h-60 mb-6" />
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div v-for="n in 8" :key="n" class="rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 animate-pulse h-48" />
        </div>
      </template>
      ...
      <template v-else>
        ...
        <router-link
          ...
          class="group block mt-16 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700 hover:shadow-lg transition-all overflow-hidden mb-6"
        >
          <div
          ...
          <div class="p-6 lg:p-8">
            <h2 class="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors leading-tight mb-3">
              {{ posts[0].title }}
            </h2>
            <p v-if="posts[0].excerpt" class="text-base text-gray-500 dark:text-gray-300 leading-relaxed mb-5 max-w-3xl">
              {{ posts[0].excerpt }}
            </p>
            <div class="flex items-center gap-4 text-sm text-gray-400">
              <span v-if="posts[0].author?.name" class="font-medium text-gray-600 dark:text-gray-300">{{ posts[0].author.name }}</span>
              <span>{{ formatDate(posts[0].published_at) }}</span>
              <span class="ml-auto text-emerald-600 dark:text-emerald-400 font-medium group-hover:translate-x-1 transition-transform inline-flex items-center gap-1">
                Read more
        ...
        <div v-if="posts.length > 1" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          <router-link
            ...
            class="group flex flex-col rounded-xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700 hover:shadow-md transition-all overflow-hidden"
          >
            <!-- Gradient header -->
            <div
            ...
            <div class="flex flex-col flex-1 px-3 py-3">
              <h2 class="text-sm font-semibold text-gray-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors leading-snug mb-1.5 line-clamp-2">
                {{ post.title }}
              </h2>

              <div class="mt-auto pt-2 flex items-center justify-between text-xs text-gray-400">
                <span class="font-medium text-gray-500 dark:text-gray-400 truncate">{{ post.author?.name }}</span>
