    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          ...
          :key="i"
          class="rounded-2xl bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-200 dark:ring-gray-700 p-6 flex flex-col gap-4"
        >
          <p class="text-sm text-gray-600 dark:text-gray-300 leading-relaxed flex-1">"{{ item.quote }}"</p>
          <div class="flex items-center gap-3">
            ...
            />
            <div v-else class="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center flex-shrink-0">
              <span class="text-sm font-semibold text-indigo-600 dark:text-indigo-400">{{ item.author?.[0]?.toUpperCase() }}</span>
            </div>
            <div>
